"""
HTR Scan Worker (Alpaca edition) -- v3.1
------------------------------------------
Reverted to REST polling after the v4 WebSocket rewrite failed silently
in production: Alpaca's free tier caps trade/quote WebSocket
subscriptions at 30 symbols, so the "*" wildcard subscription (needed to
watch the whole market) was rejected with a "symbol limit exceeded"
error immediately on connect -- and that error didn't raise an
exception or crash the process, so the worker sat connected but
subscribed to nothing, producing zero alerts for hours with no visible
failure. This version goes back to the known-working REST polling
design while a proper fix (a capped, dynamically-managed watchlist
subscription, or paying for the unlimited-symbol tier) gets built and
tested separately, rather than leaving a silently-broken version live.

Two-tier alerting:
    Tier 1 "Early Warning": %change + relative volume, no absolute
    volume floor -- fires fast, especially on thin names.
    Tier 2 "Confirmed": %change + absolute volume floor (700K default)
    -- the noise filter, confirms real participation.

Halt detection: if an already-alerted symbol's price/volume is
identical for HALT_FREEZE_CYCLES consecutive polls, fire "possible
halt"; the next poll where it changes again fires "reopened". Gated to
the regular session (9:30 AM-4:00 PM ET) only -- outside that window,
most thin names naturally go quiet for many poll cycles as a matter of
course, which produced false mass-halt clusters before this gate was
added.
...
Known limitation carried over from all prior versions: Alpaca's free
REST endpoints are 15-minute delayed (only their WebSocket is
real-time, and that's the 30-symbol-capped feature that failed above).
So this version is reliable but not real-time -- that tradeoff is
deliberate here, not an oversight, given the streaming version's
failure mode was worse than a known delay.
"""
...
def halt_detection_active():
    now = datetime.now(ET).time()
    return HALT_DETECTION_START <= now < HALT_DETECTION_END


def check_halt_state(symbol, price, day_vol):
    if not halt_detection_active():
...
def main():
    log.info(
        "Scan worker (Alpaca) v3.1 starting. Tier1: %%chg>%s relvol>%sx | Tier2: %%chg>%s vol>%s | "
        "Halt freeze=%s cycles (regular session only)",
    ...
    while True:
        try:
            ...
            log.error("run_once() raised an unhandled exception, continuing: %s", e)
